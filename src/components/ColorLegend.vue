...
const colors = computed(() => {
  ...
  // When color dimension matches row dimension, use the pre-computed mean-based ordering
  if (colorDimension === appStore.dimensions[Axis.ROW] && colorStore.orderedYCategoricalScale.length > 0) {
    // Use the pre-computed ordered scale from the store
    const orderedColors = colorStore.orderedYCategoricalScale
